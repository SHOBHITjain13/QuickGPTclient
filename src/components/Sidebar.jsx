  ...
  return (
    ...
    transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      <div className='flex items-center gap-2 p-3 mt-4 border border-gray-400
        ...
        <input onChange={(e) => setSearch(e.target.value)} value={search} type='text' placeholder='Search conversation' className='test-xs placeholder:text-gray-400 outline-none' />
      </div>
            ...
            toLowerCase().includes(search.toLowerCase()) : chat.name.toLowerCase().
              includes(search.toLowerCase())).map((chat) => (
                <div onClick={()=> {navigate('/'); setSelectedChat(chat); 
                  setIsMenuOpen(false)
                }} key={chat._id} className='p-2 px-4 dark:bg-[#5731C]/10 border 
      ...
      <div className='flex items-center justify-between gap-2 p-3 mt-4 border border-gray-300
        ...
        <label className='relative inline-flex cursor-pointer'>
          <input onChange={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
            type="checkbox" className='sr-only peer' checked={theme ===
              'dark'} />
      ...
      <div  className='flex items-center gap-3 p-3 mt-4 border border-gray-300
       ...
       <p className='flex-1 text-small dark:text-primary truncate'>{user ? user.name
